...
/// RFC 9106 Section 3.3에서 정의된 가변 출력 BLAKE2b 함수입니다 (H').
///
...
mod tests {
    ...
    // BLAKE2b keyed(MAC) 공식 KAT (key 00..3f, RFC 7693 reference blake2-kat)
    // 키드 경로(new_keyed) 의 byte-identity 회귀 가드 단일 키 블록 케이스
    #[test]
    fn blake2b_keyed_empty() {
        let key: [u8; 64] = core::array::from_fn(|i| i as u8);
        let h = Blake2b::new_keyed(64, &key);
        let digest = h.finalize().unwrap();
        let expected = [
    ...
    // 키 블록 직후 1바이트 메시지 키 블록(t=128) 비-최종 압축 + 메시지 블록(t=129) 최종 압축 경로
    #[test]
    fn blake2b_keyed_one_byte() {
        let key: [u8; 64] = core::array::from_fn(|i| i as u8);
        let mut h = Blake2b::new_keyed(64, &key);
        h.update(&[0u8]);
        let digest = h.finalize().unwrap();
        let expected = [
    ...
    // 키(32B) + 200B 메시지 키 블록 경계를 넘는 다중 블록 키드 카운터 회귀 (dlen=32)
    #[test]
    fn blake2b_keyed_multiblock() {
        ...
        let digest = h.finalize().unwrap();
        let expected = [
            0x98, 0x08, 0x23, 0x5b, 0x8b, 0xc5, 0x94, 0xf9, 0x77, 0x4c, 0x06, 0xf4, 0xbf, 0x16,
            0xe2, 0xbd, 0xd2, 0x0e, 0x51, 0x26, 0x77, 0x05, 0xee, 0x29, 0x29, 0x83, 0x5c, 0x91,
            0x4d, 0x9b, 0xce, 0xaa,
        ];
        assert_eq!(digest.as_slice(), &expected);
    }
    ...
    // blake2b_long(RFC 9106 H') byte-identity KAT
    // out_len=80 r=1 last_len=48 (V_1 32B + V_2[..48]) 체이닝 진입 경로
    #[test]
    fn blake2b_long_kat_80() {
        let out = blake2b_long(b"test", 80).unwrap();
        let expected = [
            0xf2, 0xe5, 0x95, 0xe6, 0xc0, 0xdc, 0x15, 0x97, 0xb2, 0x31, 0xa5, 0x65, 0xc0, 0xdb,
            0xf1, 0x8f, 0x02, 0x1a, 0x36, 0xed, 0x3c, 0x7d, 0x72, 0x98, 0xc3, 0x6c, 0xc0, 0xe7,
            0xe7, 0x90, 0xe2, 0x45, 0x6d, 0x1e, 0xc4, 0x5f, 0x62, 0x2c, 0xe9, 0x73, 0x78, 0x2e,
            0x02, 0x1f, 0x35, 0x46, 0xd1, 0xe8, 0xec, 0xcd, 0x41, 0x52, 0xe1, 0x14, 0x16, 0x36,
            0xce, 0xae, 0x73, 0x23, 0x29, 0x9f, 0x54, 0xad, 0xf4, 0xcd, 0x9a, 0xaf, 0x1a, 0xcf,
            0x03, 0x9d, 0xe3, 0xe0, 0x20, 0x7c, 0x73, 0x49, 0x8f, 0x37,
        ];
        assert_eq!(out.as_slice(), &expected);
    }

    // out_len=128 r=2 (V_1, V_2 각 32B + V_3 64B) 루프 1회 경로
    #[test]
    fn blake2b_long_kat_128() {
        let out = blake2b_long(b"threat-input", 128).unwrap();
        let expected = [
            ,
        ];
        assert_eq!(out.as_slice(), &expected);
    }
