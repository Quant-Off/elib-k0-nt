...
impl Poly1305 {
    ...
    pub fn new(key: &[u8; 32]) -> Self {
        ...
        let mut t0 = u32::from_le_bytes([clamped[0], clamped[1], clamped[2], clamped[3]]);
        let mut t1 = u32::from_le_bytes([clamped[4], clamped[5], clamped[6], clamped[7]]);
        let mut t2 = u32::from_le_bytes([clamped[8], clamped[9], clamped[10], clamped[11]]);
        let mut t3 = u32::from_le_bytes([clamped[12], clamped[13], clamped[14], clamped[15]]);

        let mut r0 = t0 & 0x03ff_ffff;
        let mut r1 = ((t0 >> 26) | (t1 << 6)) & 0x03ff_ffff;
        let mut r2 = ((t1 >> 20) | (t2 << 12)) & 0x03ff_ffff;
        let mut r3 = ((t2 >> 14) | (t3 << 18)) & 0x03ff_ffff;
        let mut r4 = (t3 >> 8) & 0x03ff_ffff;

        clamped.zeroize();

        let mut pad0 = u32::from_le_bytes([key[16], key[17], key[18], key[19]]);
        let mut pad1 = u32::from_le_bytes([key[20], key[21], key[22], key[23]]);
    ...
    fn block(&mut self, m: &[u8], hibit: u32) {
        let mut m0 = u32::from_le_bytes([m[0], m[1], m[2], m[3]]);
        let mut m1 = u32::from_le_bytes([m[4], m[5], m[6], m[7]]);
        let mut m2 = u32::from_le_bytes([m[8], m[9], m[10], m[11]]);
        let mut m3 = u32::from_le_bytes([m[12], m[13], m[14], m[15]]);

        let mut h0 = self.h[0].wrapping_add(m0 & 0x03ff_ffff);
        let mut h1 = self.h[1].wrapping_add(((m0 >> 26) | (m1 << 6)) & 0x03ff_ffff);
        let mut h2 = self.h[2].wrapping_add(((m1 >> 20) | (m2 << 12)) & 0x03ff_ffff);
        let mut h3 = self.h[3].wrapping_add(((m2 >> 14) | (m3 << 18)) & 0x03ff_ffff);
        let mut h4 = self.h[4].wrapping_add((m3 >> 8) | hibit);
        ...
        let mut s1 = r1.wrapping_mul(5);
        let mut s2 = r2.wrapping_mul(5);
        let mut s3 = r3.wrapping_mul(5);
        let mut s4 = r4.wrapping_mul(5);

        let mut d0 = (h0 as u64)
        ...
        d1 = d1.wrapping_add(c as u64);
        h1 = d1 as u32 & 0x03ff_ffff;
        c = (d1 >> 26) as u32;

        d2 = d2.wrapping_add(c as u64);
        h2 = d2 as u32 & 0x03ff_ffff;
        c = (d2 >> 26) as u32;

        d3 = d3.wrapping_add(c as u64);
        h3 = d3 as u32 & 0x03ff_ffff;
        c = (d3 >> 26) as u32;

        d4 = d4.wrapping_add(c as u64);
    ...
    pub fn finalize(mut self) -> [u8; 16] {
        ...
        let ge_p = Choice::from_u8(((g4 >> 31) ^ 1) as u8);
        h0 = CtSelOps::select(&h0, &g0, ge_p);
        h1 = CtSelOps::select(&h1, &g1, ge_p);
        h2 = CtSelOps::select(&h2, &g2, ge_p);
        h3 = CtSelOps::select(&h3, &g3, ge_p);
        h4 = CtSelOps::select(&h4, &g4, ge_p);

        let mut f0 = (h0 | (h1 << 26)) as u64 + self.pad[0] as u64;
